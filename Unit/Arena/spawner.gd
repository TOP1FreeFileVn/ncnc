...
func spawn_enemy():
	var enemy_scene := current_wave_data.get_random_scence() as PackedScene
	if enemy_scene:
		var spawn_pos  := get_random_spawn_position()
		var enemy_instance := enemy_scene.instantiate() as Enemy
		enemy_instance.global_position = spawn_pos
		get_parent().add_child(enemy_instance)
		spawn_enemies.append(enemy_instance)
	set_spawn_timer()
...
func _on_wave_timer_timeout() -> void:
	...
	on_wave_conpleted.emit()
	spawn_timer.stop()
	clear_enemies()
	update_enemeis_new_wave()
